    ...
    <div className="p-6 md:p-8 max-w-2xl">
      ...
      <div className="flex gap-1 mb-6 bg-gray-100 rounded-xl p-1 w-fit">
        {[
          ...
          { key: 'sifre', label: 'Şifre Değiştir', icon: Lock },
        ].map((tab) => (
          <button
            key={tab.key}
      ...
      {activeTab === 'profil' && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <div>
            ...
            <div className="px-3 py-2 bg-orange-50 border border-orange-200 rounded-md text-sm text-orange-700 font-medium inline-block">
              {session?.user?.role === 'PET_OWNER' ? '🐾 Pet Sahibi' :
               session?.user?.role === 'BUSINESS_OWNER' ? '✂️ İşletme Sahibi' : '🛡️ Admin'}
            </div>
          </div>
          <Button
            onClick={saveProfil}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          <div>
            ...
            <Input
              ...
              onChange={(e) => setSifre({ ...sifre, currentPassword: e.target.value })}
              placeholder="••••••••"
            />
          </div>
          <div>
            ...
            <Input
              ...
              onChange={(e) => setSifre({ ...sifre, newPassword: e.target.value })}
          ...
          <div>
            ...
            <Input
              type="password"
              value={sifre.confirmPassword}
              onChange={(e) => setSifre({ ...sifre, confirmPassword: e.target.value })}
              placeholder="••••••••"
            />
          </div>
          <Button
